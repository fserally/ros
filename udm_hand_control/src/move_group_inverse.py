...
class MoveGroupPythonIntefaceTutorial(object):
  ...
  def __init__(self, group_param):
    ...
    move_group = moveit_commander.MoveGroupCommander(group_param)
  ...
  def go_to_joint_state(self,joint0, joint1, joint2,group):
...
def handle_req(req):
    ...
    moveFinger = MoveGroupPythonIntefaceTutorial(req.group.data)
    #moveFinger.go_to_joint_state(req.joint0.data, req.joint1.data, req.joint2.data,req.group.data)
    cartesian_plan, fraction = moveFinger.plan_cartesian_path(scale=-1)
    moveFinger.display_trajectory(cartesian_plan)
    moveFinger.execute_plan(cartesian_plan)
    rep=move_serviceResponse()
    rep.res=True
    rep.message="success"
    return rep
...
def simple_server():
    rospy.init_node('move_service_fatimah_server')
    s = rospy.Service('move_service_fatimah', move_service, handle_req)
    rospy.loginfo("move_finger_service launched")
    rospy.spin()
